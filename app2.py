...
def radar_chart_svg(scores, size=260):
    ...
    labels = ["Operat.", "Financier", "Marche", "Resilience", "Marque", "Legal"]
...
def compute_scores(d):
    ...
    if goppar_negatif:
        ...
        # linear_score sur marge GOP — calibré benchmarks réels hôtellerie France
        ops = round(linear_score(gop_ratio, [(0,0),(15,20),(25,45),(35,72),(45,100)]))

    # Break-even occupancy = OpCost / ADR — occupation min pour couvrir les coûts
    breakeven_occ = (d['opcost'] / d['adr'] * 100) if d['adr'] > 0 else 100
    cushion_occ   = d['occ'] - breakeven_occ  # marge de sécurité en pts occupation

    # FIN score — linear_score sur les 3 métriques clés
    ltv_s  = round(linear_score(d['ltv'],    [(0,100),(50,100),(60,75),(70,40),(85,15),(100,0)]))
    dscr_s = round(linear_score(d['dscr'],   [(0.5,0),(1.0,10),(1.2,40),(1.3,65),(1.5,85),(2.0,100),(3.5,100)]))
    cr_s   = round(linear_score(d['caprate'],[(2,0),(3.5,25),(4,40),(5,65),(6,85),(8,100),(12,100)]))

    # DSCR calculé réel depuis taux d'intérêt + LTV (si disponibles)
    interest_rate   = d.get('interest_rate', 0)
    equity_invested = d.get('equity_invested', 0)
    asset_val_est   = (d['noi'] / d['caprate'] * 100) if d['caprate'] > 0 else 0
    dette_est       = asset_val_est * d['ltv'] / 100
    ds_calcule      = dette_est * interest_rate / 100 if interest_rate > 0 else 0
    dscr_calcule    = (d['noi'] / ds_calcule) if ds_calcule > 0 else 0
    dscr_incoherent = (dscr_calcule > 0 and abs(dscr_calcule - d['dscr']) > 0.35)

    # Cash-on-Cash Return = NOI / Fonds propres investis
    coc_return_v    = (d['noi'] / equity_invested) if equity_invested > 0 else 0
    coc_insuffisant = (coc_return_v > 0 and coc_return_v < 0.05)
    ...
    # RES score — intègre l'âge du bâtiment à 20%
    building_age = d.get('building_age', 20)
    # Plus le bâtiment est âgé, plus le risque capex est élevé
    age_score = round(linear_score(building_age, [(0,100),(10,95),(20,80),(30,60),(40,35),(60,10)]))
    res_base  = min(100, round(((d['esg'] + d['digi'] + (10-d['hr']) + d['ins'] + d['div']) / 50) * 100))
    res       = round(res_base * 0.80 + age_score * 0.20)
    ...
    return dict(
        ...
        breakeven_occ=round(breakeven_occ,1),
        cushion_occ=round(cushion_occ,1),
        dscr_calcule=round(dscr_calcule,2),
        dscr_incoherent=dscr_incoherent,
        coc_return_v=round(coc_return_v,3),
        coc_insuffisant=coc_insuffisant,
        age_score=age_score,
    )
...
with t2:
    ...
    with c2:
        ...
        building_age = st.number_input("Age du batiment (ans)", 0, 100,
                                        int(D.get('building_age', 20)),
                                        help="Intégré à 20% dans le score Résilience — proxy du risque Capex")
...
with t3:
    ...
    with c2:
        ...
        interest_rate   = st.number_input("Taux d'intérêt du prêt (%)", 0.0, 15.0,
                                           float(D.get('interest_rate', 3.5)), 0.1,
                                           help="Permet de calculer le DSCR réel et de détecter les incohérences")
        equity_invested = st.number_input("Fonds propres investis (k€)", 0, 200000,
                                           int(D.get('equity_invested', 2500)),
                                           help="Utilisé pour calculer le Cash-on-Cash Return")
...
try:
    ...
    data = dict(
        ...
        interest_rate=D.get('interest_rate', 3.5),
        equity_invested=D.get('equity_invested', 2500),
        building_age=D.get('building_age', 20),
...
with t1:
    ...
    # KPIs métriques — ligne 1 : performance core
    col_radar, col_kpis = st.columns([1, 2])
    with col_radar:
        st.markdown("**Radar des 6 dimensions**")
        radar_svg = radar_chart_svg(C, size=240)
    ...
    with col_kpis:
        ...
        be_color = "normal" if C['cushion_occ'] >= 5 else "inverse"
        a1.metric("Break-even occ.", f"{C['breakeven_occ']}%",
                  delta=f"Coussin +{C['cushion_occ']:.1f}pts" if C['cushion_occ']>0 else f"Deficit {C['cushion_occ']:.1f}pts",
                  delta_color=be_color,
                  help="Taux occupation minimum pour couvrir les couts operationnels = OpCost / ADR")
        a2.metric("Cash-on-Cash", f"{C['coc_return_v']:.1%}",
                  help="NOI / Fonds propres investis — rendement annuel reel sur les FP")
        dscr_delta = f"Calc: {C['dscr_calcule']:.2f}x" if C['dscr_calcule'] > 0 else "N/A"
        a3.metric("DSCR calcule", dscr_delta,
                  delta="Incoherent" if C['dscr_incoherent'] else "Coherent",
                  delta_color="inverse" if C['dscr_incoherent'] else "normal",
                  help="DSCR recalcule depuis taux d'interet + LTV — alerte si ecart > 0.35x avec DSCR declare")
        a4.metric("Age batiment", f"{data.get('building_age', 20)} ans",
                  delta="Risque capex" if data.get('building_age', 20) >= 35 else "OK",
                  delta_color="inverse" if data.get('building_age', 20) >= 35 else "normal",
                  help="Integre a 20% dans le score Resilience")
        a5.metric("Score age", f"{C['age_score']}/100",
                  help="Score de risque lie a l'age du batiment — proxy risque capex structure")
        a6.metric("TRevPAR", f"{C['trevpar']}€",
                  help="Total Revenue Per Available Room = RevPAR x 1.12 (revenus hebergement + autres)")
    ...
    if C['dscr_incoherent']:
                         flags.append(("red",   f"DSCR incoherent — DSCR declare {data['dscr']:.2f}x vs DSCR calcule {C['dscr_calcule']:.2f}x (ecart > 0.35x). Verifier taux d'interet et LTV."))
    if C['coc_insuffisant']:
                         flags.append(("amber", f"Cash-on-Cash insuffisant ({C['coc_return_v']:.1%}) — rendement annuel sur FP < 5%, en dessous du seuil institutionnel"))
    if data.get('building_age', 0) >= 40:
                         flags.append(("amber", f"Batiment age ({data.get('building_age',0)} ans) — risque capex structure eleve, prevoir audit technique et provision renovation"))
    if C['cushion_occ'] < 5:
                         flags.append(("amber", f"Coussin d'occupation faible ({C['cushion_occ']:.1f}pts) — break-even a {C['breakeven_occ']}% d'occupation, marge de securite reduite"))
